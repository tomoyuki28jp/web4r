...
(defpage ajax/list (:get item order)
  (when-let (slot (aand item (gethash it *slot-indices*)))
    (when-let (class (find-symbol (->string-up (car (split "_" item)))
                                  (symbol-package slot)))
      (let* ((type  (slot-type (get-slot class slot)))
             (sort  (if (string= "desc" (->string-down order))
                        (if (eq type 'integer) #'> #'string>)
                        (if (eq type 'integer) #'< #'string<)))
             (slots (get-excluded-slots class))
             (cname (->string-down class)))
        (when-let (items (per-page (get-instances-by-class class)
                                   :index slot :sort sort))
