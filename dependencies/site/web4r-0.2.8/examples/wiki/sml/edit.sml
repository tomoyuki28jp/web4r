(with-sml-file (sml-path "template.sml")
  (remove  link)
  (replace title [title (if oid "Editing " "New ") "wiki"])
  (append  head  [script :type "text/javascript" :src "/js/examples/showdown.js"])
  (append  head  [script :type "text/javascript" :src "/js/examples/showdown-gui.js"])
  (append  head  [script :type "text/javascript" (safe "
var validate = function() {
    if (document.wiki_form.wiki_title.value == '') {
        alert(\"title can't be empty\"); return false;
    }
    if (document.wiki_form.wiki_body.value == '') {
        alert(\"body can't be empty\");  return false;
    }
    return true;
}
")])
  (append  head  [link :rel "stylesheet" :href "/css/examples/showdown.css"
                       :type "text/css"  :charset "utf-8" /])
  (replace body
    [body
     [div :id "pageHeader" [h1 (if oid "Editing " "New ") "wiki"]
          (msgs)]
     [div :id "leftContainer"
          [form :method "post" :action "" :name "wiki_form" :id "wiki_form"
                :onsubmit "return validate()"
                [div :class "paneHeader" :style "font-size: 90%;"
                     [label :for "wiki_title" "Title"]
                     [input :type "text" :name "wiki_title" :id "wiki_title"
                            :class "required" :maxlength "256" :size "50"
                            :value (form-value "wiki_title" 'wiki::title ins) /]
                     [input :type "submit" :value "save"]]
                [textarea :style "height: 673px;" :id "inputPane" :cols "80"
                          :rows "20" :class "pane" :name "wiki_body"
                          (form-value "wiki_body" 'wiki::body ins)]
                [input :type "hidden" :name "cid" :value (set-cont edit/cont*)]]]
     [div :id "rightContainer"
          [div :class "paneHeader"
               [select :id "paneSetting"
                       [option :value "previewPane" "Preview"]
                       [option :value "outputPane"  "HTML Output"]
                       [option :value "syntaxPane"  "Syntax Guide"]]]
          [textarea :style "display: none; height: 673px;" :id "outputPane"
                    :class "pane" :cols "80" :rows "20" :readonly "readonly"]
        [div :style "display: block; height: 673px;" :id "previewPane" :class "pane"]
...
Trust [me].
  ...
  [me]: http://attacklab.net/ \"AttackLab\"
...
"]
    [div :id "footer"
        [span :id "convertTextControls"
            [button :id "convertTextButton" :type "button" :title "Convert text now" "Convert text"]
            [select :id "convertTextSetting"
                [option :value "delayed" "in the background"]
                [option :value "continuous" "every keystroke"]
                [option :value "manual" "manually"]]]
        [div :id "processingTime" :title "Last processing time" "5 ms"]]
    [script :type "text/javascript" "startGui();"]
    [script :type "text/javascript" :src "/js/examples/attacklab.js"]]]))
